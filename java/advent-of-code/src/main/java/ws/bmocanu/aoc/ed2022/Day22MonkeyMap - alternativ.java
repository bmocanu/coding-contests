...
public class Day22MonkeyMap extends SolutionBase {
    ...
    public static void main(String[] args) {
            ...
            if (command.command != null) {
                ...
                for (int step = 0; step < command.steps; step++) {
                    ...
                    if (nextPoint == null || nextPoint.type == NOTHING) {
                        Tuple tuple = getPointAroundTheTable(cursor, dir, flex);
        ...
        Log.part1(calculateScore(cursor, dir));
        Log.part2(0);
    }
    ...
    private static Tuple getPointAroundTheTable(Point cursor, PosDelta4 dir, FlexStruct flex) {
        int startX = cursor.x;
        int startY = cursor.y;
        System.out.println("MoveAround x=" + startX + ", y=" + startY + ", dir=" + dir.dir4);
        ...
        // -- going up is fine
        // -- going down
        if (cursor.x == 149 && XNum.intBetween(cursor.y, 50, 99) && dir.isDown()) {
            startX = 50;
            startY = 50 - cursor.x;
            startDir = PosDelta4.left();
        }
